...
8. Open the Google Sheet and click on the share button on the right-hand side. Update the sheet permissions and add the Service Account Identifier (The client email on the service account) as a user invited in the spreadsheet's Collaboration Settings with edit permission.
...
To get started, follow these additional steps for configuring Google Sheets in the Flows:

12. Import the sample flow [here](https://github.com/glific/docs/3.%20Flows/3.%20Flow%20Features/write_to_sheet.json) from the Flow screen and configure it.

13. Once the Flow start is imported, the flow will appear as shown.
